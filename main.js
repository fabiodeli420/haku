...
// ===== PRODUCTOS =====
// Para poner una foto real: completá "image" con la ruta del archivo,
// por ejemplo 'images/remera-oversize.jpg'. Si lo dejás vacío (''),
// se muestra el emoji + fondo de color como ahora.
const products = [
  { id:1, name:'Remera Oversize', desc:'100% algodón premium, fit holgado', price:8500, emoji:'👕', image:'', stock:12, gradient:'linear-gradient(135deg,#0b1020,#12213f)' },
  { id:2, name:'Buzo Hoodie',     desc:'Con capucha, bolsillo canguro',     price:15000, emoji:'🧥', image:'', stock:8,  gradient:'linear-gradient(135deg,#0a0e1a,#0e1c38)' },
  { id:3, name:'Pantalón Baggy',  desc:'Corte baggy, cintura elástica',     price:12000, emoji:'👖', image:'', stock:5,  gradient:'linear-gradient(135deg,#0b0f1a,#122542)' },
  { id:4, name:'Jogger Oversize', desc:'Cómodo y liviano para el día a día',price:11000, emoji:'🩳', image:'', stock:15, gradient:'linear-gradient(135deg,#0a0e18,#101f3a)' },
  { id:5, name:'Set Completo',    desc:'Buzo + Pantalón combinados',        price:24000, emoji:'✨', image:'', stock:3,  gradient:'linear-gradient(135deg,#0b1120,#142848)' },
  { id:6, name:'Trucker Cap HK',  desc:'Bordado HK premium, ajustable',     price:5500,  emoji:'🧢', image:'', stock:20, gradient:'linear-gradient(135deg,#0a0f1c,#111f3c)' },
];
...
function renderProducts() {
  ...
  grid.innerHTML = products.map((p, i) => {
    ...
    const imgInner = p.image
      ? `<img src="${p.image}" alt="${p.name}" loading="lazy" onerror="this.parentElement.innerHTML='${p.emoji}'">`
      : p.emoji;
    return `
    <div class="product-card reveal" style="animation-delay:${i*0.08}s">
      <div class="product-img" style="background:${p.gradient}">${imgInner}</div>
      <div class="product-info">
        ...
        <div class="product-bottom">
          ...
          <button class="add-btn" onclick="addToCart(${p.id})" aria-label="Agregar ${p.name} al carrito">+</button>
        </div>
      </div>
    </div>`;
  }).join('');
  ...
  items.innerHTML = cart.map(c => `
    <div class="cart-item">
      ...
      <div class="cart-item-qty">
        <button class="qty-btn" onclick="changeQty(${c.id}, -1)" aria-label="Restar unidad">−</button>
        <span>${c.qty}</span>
        <button class="qty-btn" onclick="changeQty(${c.id}, 1)" aria-label="Sumar unidad">+</button>
      </div>
    </div>`).join('');
}
...
function launchConfetti() {
  ...
  const colors = ['#2f6bff','#7aa2ff','#ffffff','#1a4fd6'];
  for (let i = 0; i < 60; i++) {
...
document.querySelector('.nav-logo').addEventListener('click', e => {
  ...
  if (logoClicks >= 5) {
    ...
    showToast('MODO NEGATIVO ACTIVADO');
